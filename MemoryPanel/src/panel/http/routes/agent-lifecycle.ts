 ...
 *      isSkillCapabilityAbsent）。其余任何 404 / 非 0 code 一律原样透传，保持 fail-closed
...
async function listAgentSkills(
  ...
  for (;;) {
    ...
    if (env.code !== 0) {
      ...
      // 这是模块级属性，现实里只可能在 offset=0 命中，已收集部分原样返回即可。
      // 其余错误保持 fail-closed：原样透传给 caller，不触碰 agent/archive。
      if (isSkillCapabilityAbsent(env)) break;
